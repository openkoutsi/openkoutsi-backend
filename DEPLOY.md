...
- A proposal holds one of `AGENT_MAX_CONCURRENT_RUNS` agent slots for up to
  150 seconds while the weeks are written — the one place a chat turn can hold a
  slot that long, so raise `AGENT_MAX_CONCURRENT_RUNS` if your instance has
  several athletes chatting at once and a slow model. It is not a figure to tune
  back down on its own: it has to outlast the nested model call's own 120-second
  budget, or the tool's wrapper wins the race and cancels a draft that has
  already been written to `plan_proposals` — leaving a proposal the answer never
  mentions.

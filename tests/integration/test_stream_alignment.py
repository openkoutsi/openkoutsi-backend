...
class TestReprocessMigratesOldStreams:
    """A reprocess is how an existing ride reaches the shared clock.

    There is no bulk migration: activities stored before issue #76 keep the
    dense streams they have, where the index is a sample rather than a second.
    Re-parsing the device file on reprocess is the route onto the new
    convention, and without it the change would never reach a single ride the
    athlete already owns.
    """

    async def _stored_dense(self, client, auth_headers, session) -> str:
        """Upload the fixture, then overwrite its streams with the old shape."""
        activity_id = await _upload_and_process(client, auth_headers, session)
        ...
        for row in (
            await session.execute(
                select(ActivityStream).where(
                    ...
                    ActivityStream.stream_type.in_(_RECORDED_CHANNELS),
                )
            )
        ).scalars():
            # Exactly what the old parser produced: gaps closed up, so each
            # channel ends up a different length and misaligned with the rest.
            row.data = [v for v in streams[row.stream_type] if v is not None]
        await session.commit()
        return activity_id

    async def test_dense_streams_start_out_ragged(self, client, auth_headers, session):
        activity_id = await self._stored_dense(client, auth_headers, session)
        streams = await _stored_streams(session, activity_id)
        # The setup is only meaningful if it reproduces the bug: power and heart
        # rate disagree by exactly the length of the strap dropout.
        assert len(streams["power"]) - len(streams["heartrate"]) == (
            HR_DROPOUT_END_S - HR_DROPOUT_START_S
        )

    async def test_reprocess_puts_them_back_on_the_clock(
        self, client, auth_headers, session
    ):
        activity_id = await self._stored_dense(client, auth_headers, session)
        ...
        served = resp.json()["streams"]
        for channel in _RECORDED_CHANNELS:
            assert len(served[channel]) == _ELAPSED_S, f"{channel} not on the grid"
        assert served["heartrate"][HR_DROPOUT_START_S] is None
        assert served["power"][HR_DROPOUT_START_S] is not None

    async def test_an_activity_with_no_fit_keeps_its_streams(
        self, client, auth_headers, session
    ):
        # A Strava-only import has nothing to re-parse. It must be recomputed
        # from the streams it has rather than losing them.
        activity_id = await self._stored_dense(client, auth_headers, session)
        before = await _stored_streams(session, activity_id)

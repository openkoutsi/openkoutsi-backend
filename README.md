...
explain the gap rather than discover it by failing. All nine tools are covered by
the five read scopes `activities:read`, `athlete:read`, `goals:read`,
`metrics:read` and `plans:read`; `athlete:export` is deliberately **not**
callable, since one call returning the whole record is the opposite of
task-shaped.

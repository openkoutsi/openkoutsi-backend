...
- **The admin user listing says whether an address is confirmed** — `GET /api/admin/users` returns `email_verified_at` beside `email`, and the console's Users tab shows it under each account. Login by email requires that stamp, so a self-serve signup nobody finished is a row that can never sign in, and until now it read in the listing exactly like a working account. Read the field together with `email`: null on an account that has no address means "nothing to confirm", not "unconfirmed"
...
- **Derived distance is judged by speed, not by metres** — a GPX or TCX carries no distance of its own, so it is summed from the coordinates, and the sum needs a rule for telling a stride of real riding from a receiver that jumped. That rule used to be a flat 500 m cap per step, which silently assumed ~1 Hz recording: on a variable-rate track — a route planner's export, or smart recording, where a long straight arrives as one step of kilometres — it deleted real distance with nothing anywhere saying so, summarising an 84 km ride as 60 km and shortening every gradient, segment and pacing split derived from it. A step is now travel when its *implied speed* is plausible (`geo.step_is_travel`, 216 km/h), which is the only test that holds at every recording rate; a track with no clock at all — a course file — falls back to a metre cap set where a teleport begins rather than where a sample interval does. Genuine glitches are still rejected: a fix in the Atlantic between two in the Baltic is not distance however long the gap before it
...
- **The garage — the bikes you own, and what they have done** — `/garage` and `GET /api/bikes` promote the bike row course recon already had (tyre width → Crr, riding position → CdA) into the thing an athlete actually owns: a starting odometer for the kilometres ridden before openkoutsi saw it, a maintenance log keyed by **component**, the accessories bolted on, and a retirement date for a bike that was sold. The same rows the route-analysis bike picker reads, deliberately — a second `garage_bikes` table would have made "bikes in the garage are bikes in the picker" a synchronisation problem instead of a fact. Rides attach themselves: a bike claims a set of cycling `sport_type` values (`default_sports`) and every ingest path — provider sync, file processing, a hand-logged ride and reprocess — runs the same `services/garage.assign_bike`, with `POST /api/bikes/assign-history` for the back catalogue. A sport belongs to **one** bike per athlete; a second claim is a 409 naming the bike that holds it, because two bikes claiming `GravelRide` has no correct resolution. Applied rather than suggested, unlike commute detection — no badge counts bikes and nothing is hidden from a prompt, and a per-bike total that counted only confirmed rides is a chore rather than a garage — so the safety property lives in `activities.bike_source` instead: `PATCH /api/activities/{id}` with a `bike_id` stamps `manual`, and no automatic pass will ever overwrite that, through a reprocess, a fresh sync, a history scan or an edit to what a bike claims. An explicit `null` is a choice too — a rental, a borrowed frame — so it is stamped the same way with a null bike; `(null, null)` is reserved for "never asked", which is the predicate automapping reads as free to fill. Distance is derived on read, never stored, and reported in two figures that are never blurred: `tracked_km` is what openkoutsi observed and `lifetime_km` adds the baseline the athlete typed. Component life falls out of the log — the gap in odometer readings between consecutive entries sharing a component, and `lifetime_km − reading` for the part currently fitted, which is how "how far have these tyres gone?" becomes answerable. Retiring a bike keeps its rides, its distance and its history while dropping it from the pickers; deleting one keeps every ride and course, unassigning rather than destroying them
- **Road surface under the course, with honest confidence** — with an optional, self-hosted Valhalla sidecar configured (`VALHALLA_URL`; off by default, tiles built and shipped by the self-hoster for their own region), a stored course is map-matched against OSM **in the background** after its segment table has already been returned, so upload keeps its latency. Segmentation then splits on surface as well as gradient and `Crr` comes from the matched class adjusted for tyre width — on asphalt, and wherever the surface is unknown, it is byte-identical to what the tyre-width curve alone produced, so enabling the sidecar never silently restates an existing plan. **Confidence is carried, never flattened**: Valhalla returns `paved_smooth` for a way carrying no surface information at all, so that class reads *inferred* — openkoutsi could not confirm a tag, not "this road is untagged" — while every other value requires an explicit `surface`/`tracktype`/`smoothness` tag and reads *confirmed*. An unrecognised value classifies as unknown rather than as a default that looks confident, and overlapping match chunks that disagree downgrade the disputed point. **A short severe sector is not noise**: the dissolve that keeps match artefacts from shattering a route judges on *severity*, not length, so a 40 m snap artefact disappears while 130 m of mud inside 40 km of asphalt keeps its own segment, its own `Crr`, a stripe on the profile and a sentence in the plan — a rider expecting tarmac needs that before the day, not during it. Unconfigured or unreachable, the feature is *absent* rather than broken: the course keeps its Stage 1 analysis and no error reaches the athlete. Courses already stored can be enriched with `POST /api/courses/{id}/surface` — no re-upload — and the whole feature sits behind the `allow_course_recon` instance switch, **off by default**, which refuses the capability rather than merely its entry point (the data export excepted, deliberately)
...
- **Commute detection** — write rules describing what your commute looks like (exact sport types, distance and duration bands, local time-of-day windows, days of week, in `app_settings.commute_rules`) and openkoutsi *suggests* the `commute` label on rides that match, for you to confirm — it never applies a guess to your own labels, because the `commuter` badge counts labelled rides and the RPE prompt skips them. Strava's own commute flag *is* applied directly: that one is your assertion, not a heuristic. Rules can be proposed from the commutes you have already labelled (`GET /api/activities/commute/proposal`, ten-ride minimum), the back catalogue scanned on request (`POST /api/activities/commute/scan`), and rides awaiting an answer listed with `?suggested_label=commute`. Editing a rule re-evaluates everything unanswered; a dismissal is permanent and survives reprocessing. No GPS is involved — openkoutsi holds no location data from rides
...
# yourself, reachable only from inside the deployment. Unset by default, and
# with it unset a course is simply solved as dry pavement, which the written
# plan says out loud. Building tiles is measured in hours and gigabytes; see the
# openkoutsi-ops repository. Course recon itself is gated separately by the
# `allow_course_recon` instance setting, which defaults **off**.

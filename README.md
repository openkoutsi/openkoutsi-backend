...
- **Changing the account's email address** — a signed-in user can move their account to a new address, or, on an invite-created account that never had one, set one for the first time (`POST /api/auth/change-email`). The new address is confirmed exactly the way signup confirms the original: a hashed, single-use, 1-hour link mailed to it, and nothing on the account moves until that link is opened. Changing costs the current password, so a borrowed session can't relocate the login identifier and the password-reset target; the *old* address is told a change was requested, which is what makes a move by somebody who has the password visible to the person who has the mailbox. The response is the same acknowledgement whether the target address is free, already someone else's, or the caller's own, so it can't be used to ask who has an account here. Sessions survive — this changes an identifier, not a credential, and `/logout-all` is still the way to clear the others
...
- **Concurrent-sync safety** — two syncs for one athlete arriving at once (a Wahoo webhook and a Strava backfill, milliseconds apart) used to be able to interleave in two places, both of them inside a *single* process. Refreshing an OAuth token is a read-modify-write spanning a network round trip, and Wahoo revokes the old refresh token as it issues the new one, so the loser of that race stored a dead token and the connection stayed broken until reconnected by hand; one caller now claims the rotation with a conditional `UPDATE` and the rest wait and read its result. The ±5-minute duplicate check was guarded only by an `asyncio.Lock`, a statement about one event loop offered in place of one about the database; it is now backed by a lease row (`db/leases.py`) that every writer of that database can see, with the lock kept in front of it as the free fast path. Login also hashes off the event loop, so one sign-in no longer stalls every other request for the duration of a bcrypt

...
- **No location data, from any format** — GPX is made of coordinates, so its parser reads them, derives distance and elevation gain, and drops them before returning: `Profile` has no field that could carry a coordinate and `ActivityStream` has no channel for one. `openkoutsi.gpx.extract_route()` is the deliberate, separately-named exception, for in-memory route analysis — it is never called on the ingestion path
...
- **Strava:** configure Strava app credentials in `.env` and deploy `strava_bridge/` to a public HTTPS URL. The bridge requires `STRAVA_CLIENT_SECRET` and rejects any webhook that is not signed with it — an unsigned request is refused (`401`), and if the secret is unset the bridge refuses every webhook (`403`) and warns at startup.

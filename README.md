...
- **Pausing sign-ups** — `signups_halted` stops self-serve signup temporarily, with a free-text `signup_halt_reason` the sign-up page shows verbatim under a localised headline. A second switch rather than a flipped `allow_self_signup`, because that one is standing policy and carries no reason: expressing a pause through it would tell people "not enabled on this instance", which is false, and lifting it would ask the admin to remember what the instance was doing before. Alone among the instance switches it stops at the *front door* rather than refusing a capability — invitations keep redeeming and a verification link already emailed still activates its account — because what a halt protects is the rate at which strangers arrive, and an invitation is the admin's own deliberate act. `POST /api/auth/signup` answers 503 with `{"code": "signups_halted"}`; an instance that does not offer self-serve signup at all still answers 404, so it never discloses that it is also halted
...
- **Recorded average speed** — every ingest path already stored the ride's own `avg_speed_ms` (the mean of the speed samples the file or provider carried); the activity list and detail responses now return it. It is deliberately not synthesised from distance ÷ duration when a ride has no speed channel — the two disagree on any ride with stops in it, so the field stays null and the client decides whether to show a derived figure and say so
...
- **Plan auto-close** — a plan closes itself into `completed` once its last scheduled day has passed, judged against the athlete's own local date and settled on the next read or upload. It is the same rule the "Plan finisher" badge uses, so the badge and the status cannot disagree. `completed_at` records when it happened and survives a reopen, so reactivating a finished plan sticks; editing or regenerating the plan's dates clears it, letting the plan finish again on its new end date. A finished plan keeps its adherence series, still accepts a late upload against its own dates, and is still shown to the AI coach — only `archived` is held back
- **Plan archiving/unarchiving** — creating a plan only archives existing active plans whose dates overlap it, so plans covering different periods stay active together. Archived and completed plans reactivate via `POST /api/plans/{id}/unarchive`, which archives any overlapping active plan in turn
...
- **Plan adherence score** — two deterministic, always-on scores: a per-workout **match score** (0–100) grading how well the linked activities hit target Load and duration — over- and under-performing both hurt, though a completed workout never scores below 50 — and a Load-weighted **plan adherence score** (0–100) rolling those up over the elapsed portion of a plan, with missed sessions counting zero and skips softened by reason. Computed continuously (webhook/manual ingest and the first read of the day) and persisted as a daily snapshot per plan for charting via `GET /plans/{id}/adherence`, from the plan's start date through its last day (or today, whichever comes first). The series self-heals: stored days made stale by retroactive changes are rewritten on the next recompute, a finished plan included. The live adherence summary also reports **remaining sessions** from today onward
...
# process-wide (issue #68). Importing a long history is FIT parsing and bests
# computation on the event loop, so an unpaced one makes the instance feel
# broken for as long as it lasts. Activities already imported are skipped
# without pacing, so resuming a stopped import is not slowed by this.
# 0 disables pacing. Default 60.
...
- **A sync that stops early says so, and the next one continues from there** (issue #68) — a backfill has four ways to end before it reaches the end of an athlete's history (the safety limit, a provider that stops serving detail data, a `429`, a lost lease) and all four used to return exactly what a finished run returned, so an import that stopped at 4 000 of 12 000 rides was indistinguishable from one that imported all 4 000 there were. Each run now writes a `ProviderSyncState` row — one per (user, provider), in the user's own database — recording whether it finished, which reason stopped it, how many activities it imported and listed, how far back it reached, and **how many runs in a row have ended the same way**, which is the fact no single run can produce: stopping in the same place three times is a provider problem or a poisoned range of activities. `GET /api/integrations/status` returns it per provider, `more_expected` being the field to act on without learning the vocabulary. A stop also records the page it stopped on: the next run walks the front of the history until it meets a page with nothing to do on it and then jumps to that page, so fresh quota is spent on new history instead of re-listing thousands of rides it already holds. The jump waits while any source is still owed its detail data, since only a walk past it repairs it, and a completed run clears the cursor so the next one walks everything again
- **A historical import is paced so it does not take the instance with it** (issue #68) — a backfill is the heaviest thing the server does to itself: every new ride is a provider round trip, a FIT parse, a stream decode and a bests computation, and the parse and the computation are synchronous CPU. `SYNC_ACTIVITIES_PER_MINUTE` (default 60, `0` disables) bounds how many activities one import may *work on* per minute, process-wide rather than per sync — two athletes importing at once is twice the CPU, and a bound each of them gets in full is not a bound. Only activities that actually cost something are paced: an activity already imported is skipped without a provider call or a parse, so a resumed import walks back to where it left off at full speed
